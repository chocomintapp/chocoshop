...
export const Shop: React.FC = () => {
  ...
  const [letter, setShop] = React.useState("");
  ...
  const sendShop = async () => {
    ...
    openLoadingOverlay();
    try {
      const { chocoshopContract, explore } = getContractsForChainId(chainId as ChainId);
      ...
      const { hash } = await chocoshopContract
        .connect(signer)
        .sendShop(sendTo, ethers.utils.formatBytes32String(letter));
      closeLoadingOverlay();
      openMessageModal({
  ...
  return (
    <section>
      <form className="mb-4">
        ...
        <FormInput type="text" placeholder="letter" value={letter} setState={setShop} />
      </form>
      <div className="w-32 mx-auto">
        <Button onClick={sendShop}>Send</Button>
      </div>
    </section>
  );
};
